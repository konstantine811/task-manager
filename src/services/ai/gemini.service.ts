...
Якщо в контексті є задачі з шаблону: «початок о HH:MM на годиннику» — час доби (не тривалість); «тривалість: …» — хвилини/години роботи за раз. Не трактуй 14:00 як 14 годин навантаження.
...
У контексті шаблонних задач (якщо передано): рядки з «на годиннику» / «початок о HH:MM» означають ЧАС ДНЯ (коли зробити), а не тривалість. 14:00 — це дві години дня, не 14 годин роботи. Рядки з «тривалість» — планова довжина одного виконання. Для тижневого навантаження рахуй лише «тривалість» і кількість днів у розкладі, не плутай з часом на годиннику.`;

const ADVISOR_TASKS_ONLY_SYSTEM = `Ти формуєш список задач на основі поради. Поверни JSON лише з полем "tasks" — масив задач. Кожна: title, priority ("low"|"medium"|"high"), time (тривалість у хвилинах, не час доби), category з [${CATEGORY_LIST}], whenDo — масив 1-7 опційно. У контексті шаблону «початок о HH:MM на годиннику» — це час старту, не довжина задачі.`;
